...
namespace VueJS.Services.Concrete
{
    public class SettingManager : ManagerBase, ISettingService
    {
        ...
        {
            var generalSetting = await UnitOfWork.GeneralSettings.FirstOrDefaultAsync(gs => gs.Logo, gs => gs.MobileLogo, gs => gs.Icon);
            if (generalSetting == null) return new DataResult<GeneralSettingUpdateDto>(ResultStatus.Error, "Genel ayarlar yüklenirken bir hata oluştu. Hata devam ederse lütfen yönetici ile iletişime geçiniz.", null);
        ...
        public async Task<IDataResult<GeneralSettingDto>> GeneralSettingUpdateAsync(GeneralSettingUpdateDto generalSettingUpdateDto, int userId)
        {
            ...
            if(oldGeneralSetting == null) return new DataResult<GeneralSettingDto>(ResultStatus.Error, "Genel ayarlar güncellenirken bir hata oluştu. Hata devam ederse lütfen yönetici ile iletişime geçiniz.", null);
            ...
            return new DataResult<GeneralSettingDto>(ResultStatus.Success, "Genel ayarlar başarılı bir şekilde güncellendi!", new GeneralSettingDto { GeneralSetting = updatedGeneralSetting });
        }
